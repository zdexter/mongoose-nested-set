...
//          console.log('WARNING: tree is not built for ' + modelName + ' nodes. Parent does not have lft/rgt');
  ...
  schema.static('rebuildTree', function(parent, left, callback) {
    ...
    model.find({parentId: parent._id}, function(err, childNodes) {
      if (err) return callback(err);
      if (!childNodes) return callback(new Error(modelName + ' not found'));

      if (childNodes.length > 0) {
        async.forEachSeries(childNodes, function(item, cb) {
  ...
  // Returns true if the node is a leaf node (i.e. has no childNodes)
  ...
  schema.method('selfAndAncestors', function(callback) {
    var self = this;
    var model = this.model(modelName);
    model.where('lft').lte(self.lft).where('rgt').gte(self.rgt).exec(callback);
  });
  ...
  schema.method('childNodes', function(callback) {
    var self = this;
    var model = this.model(modelName);
    model.find({parentId: self._id}, callback);
  });

  // Returns the list of childNodes + current node
  schema.method('selfAndChildNodes', function(callback) {
    var self = this;
    self.childNodes(function(err, nodes) {
      if (err) {
        callback(err, null);
      } else {
  ...
  schema.method('selfAndDescendants', function(callback) {
    var self = this;
    var model = this.model(modelName);
    model.where('lft').gte(self.lft).where('rgt').lte(self.rgt).sort({'lft': 1}).exec(callback);
  });
